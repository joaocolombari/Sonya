...
defaultanswer={'1','4000', '', '', '4', num2str(inicialSol)};
...
if (length(entrada) == 6)
   ...
   % inicializa os geradores para se repetir resultados
    ...
    if str2double(entrada{1}) == 1
        optimtool(optimproblem);
           ...
           cont = 0; contsuc = 0; Best.score = Inf('double'); Best.scoreT = Inf('double'); scorePlot = [ ; ];
           ga(optimproblem);
          end;
    end;
 ...
 case 'PS'
    ...
    for i=1:length(genesLB)
       if (dif(i) ~= 0)  
          optimproblem.x0(j)= (inicialSoln(i) - genesLB(i))/dif(i);
          if (optimproblem.x0(j)> 1) optimproblem.x0(j) =1;
            elseif (optimproblem.x0(j)<0) optimproblem.x0(j) =0;
          end;
          j=j+1;
       end;
    end;  
