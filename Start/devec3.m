function [bestmem,bestval,nfeval] = devec3(fname,VTR,D,XVmin,XVmax,y,NP,itermax,initX0, F,CR,strategy,refresh);
...
if nargin<9,  initX0 = XVmin + rand(1,D).*(XVmax - XVmin); else
  if length(initX0)~=D; err(1,length(err)+1)=9; end; end;
if nargin<10, F = 0.2; else
  if length(F)~=1; err(1,length(err)+1)=10; end; end;
if nargin<11, CR = 0.5; else
  if length(CR)~=1; err(1,length(err)+1)=11; end; end; 
if nargin<12, strategy = 2; else
  if length(strategy)~=1; err(1,length(err)+1)=12; end; end;
if nargin<13, refresh = 1; else
  if length(refresh)~=1; err(1,length(err)+1)=13; end; end; 
if length(err)>0
...
%[fsc, fsci]  = feval(fname,pop(ibest,:),y);
[fsc, fsci]  = feval(fname,pop(ibest,:)); val(1) = fsc;
nfeval  = nfeval + 1;

bestval = val(1);                 % best objective function value so far

for i=2:NP                        % check the remaining members
 %[fsc, fsci] = feval(fname,pop(i,:),y); 
 [fsc, fsci] = feval(fname,pop(i,:));  val(i) = fsc;
  nfeval  = nfeval + 1;
...
%-----Select which vectors are allowed to enter the new population------------
  for i=1:NP
    ui(i,:) = max(min(ui(i,:), 1), 0);  
   %[fsc, fsci] = feval(fname,ui(i,:),y);       
    [fsc, fsci] = feval(fname,ui(i,:)); tempval = fsc;  % check cost of competitor
    nfeval  = nfeval + 1;
...
%----Output section----------------------------------------------------------
  ...
  if (refresh > 0)
    if (rem(iter,refresh) == 0)
       fprintf(1,'______ Iteration: %d,  BestScore: %f,  F: %f,  CR: %f,  NP: %d _____________\n',iter, bestval, F,CR,NP);
        for n=1:D
