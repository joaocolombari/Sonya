function [X,FVAL,EXITFLAG,OUTPUT] = PSO(FUN,X0,LB,UB,MAXEVAL,SWARMSIZE, OPTIONS,varargin)
...
for i = 1:OPTIONS.MAX_ITER,
    ...
    randx = rand(OPTIONS.SWARM_SIZE,NDIM);
    %Vden = OPTIONS.COGNITIVE_ACC.*randx + OPTIONS.SOCIAL_ACC.*(1-randx);
    VELOCITIES(:,:,i+1) = (OPTIONS.ConstrictionFactor.*(VELOCITIES(:,:,i) + OPTIONS.COGNITIVE_ACC.*randx.*(PBEST(:,:,i)-SWARM(:,:,i)) + OPTIONS.SOCIAL_ACC.*(1-randx).*(repmat(GBEST(i,:),[[OPTIONS.SWARM_SIZE 1 1],1])-SWARM(:,:,i))));%./Vden;
    
    % update particle positions
    
    SWARMtemp(:,:) = SWARM(:,:,i)+VELOCITIES(:,:,i+1);
    xr=rand(SWARMSIZE,1)*0.5;
    %xN=icdf('normal', xr, 0, 1);
    xN = sqrt(log(1./xr));
    for j=1:OPTIONS.SWARM_SIZE,
     %z=rand(NDIM, 1)*2-1;  
     z=randn(NDIM, 1);  
     z = z*abs(xN(j))/sqrt(((z.')*z));
     sigma=abs(VELOCITIES(j,:,i+1))*alfa;
     %sigma=min(sigma, 0.1);
     sigmin=sqrt((VELOCITIES(j,:,i+1)*(VELOCITIES(j,:,i+1).')))/7;
     sigma=max(sigma, sigmin);
     SWARM(j,:,i+1) = SWARMtemp(j,:)+(z.').*sigma;
     sigma=sqrt(VELOCITIES(j,:,i+1)*(VELOCITIES(j,:,i+1).'))*alfa;
     %SWARM(j,:,i+1) = SWARMtemp(j,:)+(z.')*sigma;
    end;
