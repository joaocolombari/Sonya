function [X,FVAL,EXITFLAG,OUTPUT] = PSO(FUN,X0,LB,UB, Npar, NparSel, populacaoTr, layertype, neuronlayer, MAXEVAL, OPTIONS,varargin)
...
DEFAULT_OPTIONS = PSOSET('SWARM_SIZE', Npar,... % number of particles in swarm is equal the number of variables to be optimized
             ...
             'MAX_FUN_EVALS',MAXEVAL,...        % maximum number of function evaluations
...
for i = 1:OPTIONS.MAX_ITER,
  ...
  % ordena a populacao
    ...
    %net = newff(populacaoT, log(scoreT), neuronlayer, 'trainbr');
    ...
    populacaoT = [SWARM(indice(:), :, i)', populacaoT];
    scoreT     = [score(indice(:)), scoreT,];
    end;
