...
for i=1:(numger+1)
  ...
  for j=1:size(populacaoT, 2)
   if (abs(populacaoT(:,j)-melhor) < step/(red*red))
...
% gera mais individuos para populacao
  ...
  stepP = min((melhor(:)+step), 1) - melhor;
  stepN =  melhor - max((melhor(:)-step), 0);
  
  for j=1:(100*numparam)
    for k=1:numparam
      ind(k,1) = rem(randn()*0.5, 1);
      if ind(k,1) > 0 
           ind(k,1) = ind(k,1)*stepP(k, 1);
      else ind(k,1) = ind(k,1)*stepN(k, 1);
      end;
    end;  
