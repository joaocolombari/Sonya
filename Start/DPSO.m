function [X,FVAL,EXITFLAG,OUTPUT] = DPSO(FUN,X0,LB,UB,MAXEVAL,SWARMSIZE, snap, OPTIONS,varargin)
...
if ~isfloat(MAXEVAL),
    error('''MAXEVAL'' incorrectly specified in ''PSO''');
end
if ~isfloat(SWARMSIZE),
    error('''SWARMSIZE'' incorrectly specified in ''PSO''');
end
if ~isfloat(snap),
    error('''snap'' incorrectly specified in ''PSO''');
end
...
if length(X0) ~= length(snap),
    error('''snap'' and ''X0'' have incompatible dimensions in ''PSO''');
end
...
DEFAULT_OPTIONS = PSOSET('SWARM_SIZE', SWARMSIZE,... % number of particles in swarm is equal the number of variables to be optimized
...
% round the initial particle position (10/01/2018)
for i = 1:OPTIONS.SWARM_SIZE,
       for j = 1:NDIM,
        if snap(j)~= 0 SWARM(i,j,1) = round(SWARM(i,j,1)/snap(j))*snap(j); end;
       end;
end
...
for i = 1:OPTIONS.MAX_ITER,
    ...
    % round the particle position (10/01/2018)
    for j = 1:OPTIONS.SWARM_SIZE, 
       for k = 1:NDIM,
        if snap(k)~= 0 SWARM(j,k,i+1) = round(SWARM(j,k,i+1)/snap(k))*snap(k); end;
       end;
    end
    ...
    for j = 1:OPTIONS.SWARM_SIZE,
        for k = 1:NDIM,
            ...
            if length(UB) == 1,
                if SWARM(j,k,i+1) > UB,
                    SWARM(j,k,i+1) = min(round((UB-rand*(SWARM(j,k,i+1)-UB))/snap(k))*snap(k), UB);
                    VELOCITIES(j,k,i+1) = SWARM(j,k,i+1)-SWARM(j,k,i);
                end
            else
                if SWARM(j,k,i+1) > UB(k),
                    SWARM(j,k,i+1) = min(round((UB(k)-rand*(SWARM(j,k,i+1)-UB(k)))/snap(k))*snap(k), UB(k));
                    VELOCITIES(j,k,i+1) = SWARM(j,k,i+1)-SWARM(j,k,i);
                end
            end
            ...
            if length(UB) == 1,
                if SWARM(j,k,i+1) < LB,
                    SWARM(j,k,i+1) = max(round((LB+rand*(LB-SWARM(j,k,i+1)))/snap(k))*snap(k), LB);
                    VELOCITIES(j,k,i+1) = SWARM(j,k,i+1)-SWARM(j,k,i);
                end
            else
                if SWARM(j,k,i+1) < LB(k),
                    SWARM(j,k,i+1) = max(round((LB(k)+rand*(LB(k)-SWARM(j,k,i+1)))/snap(k))*snap(k), LB(k));
                    VELOCITIES(j,k,i+1) = SWARM(j,k,i+1)-SWARM(j,k,i);
                end
            end
        end
