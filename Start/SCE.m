...
DEFAULT_OPTIONS = SCESET('nCOMPLEXES',3,...        % number of complexes
                         'nITER_INNER_LOOP',10,... % number of iteration in inner loop (CCE algorithm)
...
OPTIONS.nPOINTS_COMPLEX = 3*NDIM+1;
%OPTIONS.nPOINTS_COMPLEX = 12
...
for i = 2:OPTIONS.MAX_ITER,
    ...
    for j = 1:OPTIONS.nCOMPLEXES,
        ...
        for k = 1:OPTIONS.nITER_INNER_LOOP,
           ...
           fprintf('>>>>>>  interaction = %d  complex = %d  interLoop = %d  <<<<<<<<<<\n',  nITERATIONS, j, k);
           figure(119); bar( [nITERATIONS, j, k], 0.5); 
           xlabel(' interaction(1)            complex(2)      interLoop(3)'); 
