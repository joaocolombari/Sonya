function [X,FVAL,EXITFLAG,OUTPUT] = PSO(FUN,X0,LB,UB, Npar, NparSel, populacaoTr, layertype, neuronlayer, OPTIONS,varargin)
...
%   unbounded above.
%   Npar is the number of particles
%   NparSel is the number of particle evaluated
%   populacaoTr is the nunber of training population
%   layerstype is the type of intermidiare layer
%   
...
DEFAULT_OPTIONS = PSOSET('SWARM_SIZE', Npar,... % number of particles in swarm is equal the number of variables to be optimized
             ...
             'MAX_FUN_EVALS',4000,...        % maximum number of function evaluations
...
for i = 1:OPTIONS.MAX_ITER,
   ...
   if inicio 
    for j = 1:OPTIONS.SWARM_SIZE,
        ...
        populacaoT(:, j) = SWARM(j,:,i)';
        scoreT(j) =  FITNESS(j,i);
    end;
    inicio = 0;
    indice = [1: OPTIONS.SWARM_SIZE];
    % inicializa o neuronet
    net = newff(populacaoT, log(scoreT), neuronlayer);
    ...
    indice = [1 : NparSel+1];
    score = inf(1, NparSel+1);
       
    for j = 1:OPTIONS.SWARM_SIZE,
        score(NparSel+1)= sim(net, SWARM(j,:,i)');
        indice(NparSel+1) = j;
        t= NparSel+1;
        while (t >1)
         if (score(t) < score(t-1))
           aux1 = indice(t); aux2 = score(t); 
           indice(t)  = indice(t-1); score(t)  = score(t-1); 
           indice(t-1)  = aux1;  score(t-1)  = aux2;
           t=t-1;
          else t = 0;    
         end;    
        end;
    ...
    indice = indice(1, 1:NparSel);
    score = score(1, 1:NparSel);
    FITNESS(:,i) = FITNESS(:,i-1);
    
    for j = 1: NparSel,
        k = indice(j);
        FITNESS(k,i) = CALCULATE_COST(FUN,SWARM(k,:,i),LB,UB,varargin{:});
        score(k) = FITNESS(k,i);
    end
    
    populacaoT = [populacaoT, SWARM(indice(:), :, i)'];
    scoreT     = [scoreT, score(indice(:))];
    end;
    
    % ordena a populacao
    for j=(size(populacaoT, 2) - size(indice, 2) + 1):size(populacaoT, 2)
    t = j;
    while (t >1)
   ...
   % identify particle's location at which the best FITNESS has been achieved (PBEST) 
    ...
    if strcmp(OPTIONS.DISPLAY,'iter'),
        ...
        if nITERATIONS == 1,
           fprintf(' Nr_Iter= %.2g    Nr_Fun_Eval = %.2g       Current_best_func = %.2g      Current_worst_func =%.2g      Best_function %.2g \n',nITERATIONS,nFUN_EVALS,min(FITNESS(:,i)),max(FITNESS(:,i)),GBEST_FITNESS(i));
        else
             fprintf(' Nr_Iter= %.2g    Nr_Fun_Eval = %.2g       Current_best_func = %.2g      Current_worst_func =%.2g      Best_function %.2g \n',nITERATIONS,nFUN_EVALS,min(FITNESS(:,i)),max(FITNESS(:,i)),GBEST_FITNESS(i));
        end
        fprintf('______________________________________________________________________________________________\n'); 
    ...
    if (toc/60 > OPTIONS.MAX_TIME) || (nFUN_EVALS >= OPTIONS.MAX_FUN_EVALS),
        if strcmp(OPTIONS.DISPLAY,'iter'),
