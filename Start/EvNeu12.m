...
% nesta solucao usa-se a matriz de correlacao para gerar a 
% a populacao aleatoria. Com isso espera-se acelerar a
% velocidade
...
% nova geracao 
 ...
 U = chol(Cor);
   ...
   for j=1:populacao1
        ind = (rand(1, numparam)*U - 0.5)*step;
        ind = melhor'+ind;
        ind = max(min( ind(:), 1), 0);
