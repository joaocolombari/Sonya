...
% faz o b variar sendo maior no inicio, converge mais lentamente, e menor         
% ao fim; Ito é AQPSO (adaptative)
...
% Local atractor 
        ...
        %u = abs(randn(1,D)); % Gaussian QPSO (Coelho, 2010) Original for Delta Quantun
        u = min(abs(randn(1,D)), 1); % Gaussian QPSO (Coelho, 2010), first modification for Gaussian Quantum
        %u = min(max(randn(1,D)*0.2+0.5, 0.001), 1); % Gaussian QPSO (Coelho, 2010) second modification for Gaussian Quantum
